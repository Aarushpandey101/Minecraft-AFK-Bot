...
function createBot() {
    ...
    bot.once('spawn', () => {
        ...
        // Trigger sleep quickly when night starts (no need to wait for main loop delay)
        bot.on('time', () => {
            if (!config.utils['auto-sleep'].enabled) return;
            if (sleepAttemptInProgress) return;
            if (bot.isSleeping) return;

            const shouldSleepNow = bot.time.isNight || bot.isRaining;
            if (shouldSleepNow) {
                trySleep(bot).catch(() => {});
            }
        });
...
async function trySleep(bot) {
    if (!config.utils['auto-sleep'].enabled || bot.isSleeping || sleepAttemptInProgress) return false;

    const shouldSleepNow = bot.time.isNight || bot.isRaining;
    if (!shouldSleepNow) return false;
    ...
    try {
        const now = Date.now();
        if (now - sleepAnnouncedAt > 30000) {
            bot.chat('sleeping...');
            sleepAnnouncedAt = now;
        }

        const sleepDistance = config.utils['auto-sleep']['approach-distance'] || 2;
        await bot.pathfinder.goto(new goals.GoalNear(bed.position.x, bed.position.y, bed.position.z, sleepDistance));
        await bot.sleep(bed);
        logger.info('Sleeping in bed.');
        return true;
    } catch (e) {
        logger.warn(`Could not sleep yet: ${e.message}`);
        return false;
    } finally {
...
function startBrainLoop(bot) {
    ...
    setTimeout(async () => {
        ...
        if (config.utils['auto-sleep'].enabled && (bot.time.isNight || bot.isRaining)) {
            const slept = await trySleep(bot);
            if (slept) {
...
function startChatLoop(bot) {
    ...
    setTimeout(() => {
        const onlyWhenPlayers = config.utils['chat-messages']['only-when-players-online'] || false;
        const otherPlayersOnline = bot?.players && Object.keys(bot.players).length > 1;

        if (!onlyWhenPlayers || otherPlayersOnline) {
